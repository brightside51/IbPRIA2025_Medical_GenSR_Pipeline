...
G.load_state_dict(torch.load('checks_128_cross/G_64_iter200001.pth'))
...
for s in range(100):
    ...
    if value_distmean > max_value_MMD:  #Save the 2 images that resulted the max MSSIM
        ...
        samples_dir = "max_MMD_real"
        if not os.path.exists(samples_dir):
            # If it doesn't exist, create the directory
            os.makedirs(samples_dir)
        featmask = np.squeeze(0.5*y[0]+0.5).data.cpu().numpy()
        c=0
        for img in featmask:
            #plt.imsave(f'{samples_dir}/slice{c}.png', img, cmap='gray')
            c=c+1
        
        samples_dir = "max_MMD_fake"
        if not os.path.exists(samples_dir):
            # If it doesn't exist, create the directory
            os.makedirs(samples_dir)
        featmask = np.squeeze(0.5*x[0]+0.5).data.cpu().numpy()
        c=0
        for img in featmask:
            #plt.imsave(f'{samples_dir}/slice{c}.png', img, cmap='gray')
            c=c+1
