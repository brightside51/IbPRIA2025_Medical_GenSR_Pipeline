...
class Dataset_Duke(Dataset):
    def __init__(self, image_size = 128, num_frames=64, min_slices=False) :
        ...
        #print(len(self.image_dirnames_Duke))
        ...
        #self.all_image_dirnames =  self.image_dirnames_Duke
    ...
    def convert_dcm_png(self, name):
        ...
        #final_image.save(f"original_{im_position}.png")
    ...
    def __getitem__(self, sampleIdx):
        ...
        for img in slides:
            ...
            #self.save_tensor_as_image(tensors[int(position)-1], f'transformed_image_{position}.png')        #vizualization for dataset
            
        tensors_filt = list(filter(lambda item: item is not None, tensors))

        max_size = self.num_frames
        num_mris = len(tensors_filt)
        duplicated_list =[]                     #only for 128x128x128
        #print(num_mris)
        if self.min_slices:
        ...
        tensors_tuple =  tuple(duplicated_list)     #only for 128x128x128
    ...
    def save_tensor_as_image(self, tensor, file_name):
