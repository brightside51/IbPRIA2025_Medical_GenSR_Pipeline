...
class Discriminator(nn.Module):
    def __init__(self, channel=512, out_class=1, is_dis=True):
        ...
        self.conv1 = nn.Conv3d(1, channel//8, kernel_size=(4, 4, 4), stride=(2, 2, 2), padding=(1, 1, 1))
        self.conv2 = nn.Conv3d(channel//8, channel//4, kernel_size=(4, 4, 4), stride=(2, 2, 2), padding=(1, 1, 1))
        self.bn2 = nn.BatchNorm3d(channel//4)
        self.conv3 = nn.Conv3d(channel//4, channel//2, kernel_size=(4, 4, 4), stride=(2, 2, 2), padding=(1, 1, 1))
        self.bn3 = nn.BatchNorm3d(channel//2)
        self.conv4 = nn.Conv3d(channel//2, channel, kernel_size=(4, 4, 4), stride=(2, 2, 2), padding=(1, 1, 1))
        self.bn4 = nn.BatchNorm3d(channel)
        self.conv5 = nn.Conv3d(channel, n_class, kernel_size=(1,4,4), stride=1, padding=0)
...
class Generator(nn.Module):
    def __init__(self, noise:int=1000, channel:int=64):
        ...
        self.relu = nn.ReLU()
        self.noise = noise
        self.tp_conv1 = nn.ConvTranspose3d(noise, _c*8, kernel_size=(4,8,8), stride=1, padding=0, bias=False)
    ...
    def forward(self, noise):
        ...
        #h = self.fc(noise)
        #h = h.view(-1, 512, 4, 8, 8)  # Adjusted for 26 slices
        h = self.tp_conv1(noise)
        h = F.relu(self.bn1(h))

        h = F.interpolate(h, size=(8, 16, 16))  # Upsample to (8, 16, 16)
        h = self.tp_conv2(h)
        h = F.relu(self.bn2(h))
        
        h = F.interpolate(h, size=(16, 32, 32))  # Upsample to (16, 32, 32)
        h = self.tp_conv3(h)
        h = F.relu(self.bn3(h))

        h = F.interpolate(h, size=(32, 64, 64))  # Upsample to (32, 64, 64)
        h = self.tp_conv4(h)
        h = F.relu(self.bn4(h))

        h = F.interpolate(h, size=(26, 64, 64))  # Upsample to (26, 64, 64)
        h = self.tp_conv5(h)
