...
def msssim_3d(img1, img2, window_size=11, size_average=True, val_range=None, normalize=False):
    ...
    weights_np = weights.cpu().numpy()  # Convert weights tensor to NumPy array
    ...
    #mssim.append(sim)
    #mcs.append(cs)
    #img1 = F.avg_pool3d(img1, (1, 4,4))        # This is for 26 slices
    #img2 = F.avg_pool3d(img2, (1, 4,4))
    #img1 = F.avg_pool3d(img1, (1, 2,2))        # This is for 128x128 slices
    #img2 = F.avg_pool3d(img2, (1, 2,2))
    for _ in range(levels):
    #for _ in range(4):
        sim, cs = ssim_exact(img1.data.cpu().numpy(), img2.data.cpu().numpy())
        mssim.append(sim)
        mcs.append(cs)
        img1 = F.avg_pool3d(img1, (2, 2,2))        # This is for 64 slices
    ...
    # Assuming mcs and mssim are NumPy arrays
    mcs_clipped = np.clip(mcs, 0, None)  # Clip negative values to zero
    mssim_clipped = np.clip(mssim, 0, None)  # Clip negative values to zero

    # Perform exponentiation after clipping
    pow1 = mcs_clipped ** weights_np
    pow2 = mssim_clipped ** weights_np
    
    # Convert pow1 and pow2 to PyTorch tensors
    pow1_tensor = torch.from_numpy(pow1)
    pow2_tensor = torch.from_numpy(pow2)
    # From Matlab implementation https://ece.uwaterloo.ca/~z70wang/research/iwssim/
    output = torch.prod(pow1_tensor[:-1] * pow2_tensor[-1])
    return output
