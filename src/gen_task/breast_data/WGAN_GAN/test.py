...
#G.load_state_dict(torch.load('checkpoint_cross_80k_64/G_iter20001.pth'))
G.load_state_dict(torch.load('checkpoint_cross_200k_128/G_128_iter350001.pth'))        #checkpoint 
#G.load_state_dict(torch.load('checkpoint_200k_cross_64/G_64_iter200001.pth'))
...
for k in range(1):
    for i, dat in enumerate(train_loader):
        ...
        sum_ssim += msssim
        mssim_scores.append(msssim.item())  # Collect MSSIM scores

    mean_mssim = sum_ssim / ((k + 1) * (i + 1))
    print(f"MSSIM real: {mean_mssim}")

# Calculate the standard deviation of MSSIM scores
std_mssim = np.std(mssim_scores)
print(f"Standard deviation for MSSIM real: {std_mssim}")
