...
class Dataset_Duke(Dataset):
    def __init__(self, root_path, image_size = 64, num_frames=64, min_slices=False) :
        ...
        self.image_dirnames = [f.path for f in os.scandir(root_path) if f.is_dir() and f.name != exclude_name and f.name != exclude_name_2 and not any(f.name.startswith(prefix) for prefix in exclude_names_Metabrest)] #Gather names of directories with images (use everything)
...
#number = trainset.get_minimum_images(path_slurm)
