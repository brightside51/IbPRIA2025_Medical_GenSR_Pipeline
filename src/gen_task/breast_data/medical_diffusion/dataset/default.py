...
class DEFAULTDataset(Dataset):
    ...
    def __init__(self, root_dir: str, image_size = 256, num_frames=32, min_slices=True) :
        super().__init__()
        self.root_path = root_dir # folder with mri
        self.image_dirnames = [f.path for f in os.scandir(root_dir) if f.is_dir() and f.name != exclude_name and f.name != exclude_name_2 and f.name != exclude_name_3  and not any(f.name.startswith(prefix) for prefix in exclude_names_Metabrest)] #Gather names of directories with images (use everything)
    ...
    def ordered_sampling(self, images, num_samples):
        if num_samples <= 0 or not images:
            return []
        ...
        if remainder == 0:
            sampled_images = [images[i * step] for i in range(num_samples)]
        else:
            sampled_images = [images[i * (step + 1)] if i < remainder else images[(remainder * (step + 1)) + (i - remainder) * step] for i in range(num_samples)]
...
#trainset = DEFAULTDataset(path_local)    
