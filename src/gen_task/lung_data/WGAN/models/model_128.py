...
class Discriminator(nn.Module):
    def __init__(self, channel=512):
        ...
        self.conv5 = nn.Conv3d(channel, n_class, kernel_size=4, stride=1, padding=0)  # Adjust stride and padding
...
class Generator(nn.Module):
    def __init__(self, noise:int=1000, channel:int=64):
        ...
        self.noise = noise
        self.fc = nn.Linear(1000, 512*4*8*8)  # Adjusted output size for 8x8x8 feature maps
        ...
        self.tp_conv5 = nn.ConvTranspose3d(64, 1, kernel_size=4, stride=2, padding=1, bias=False)  # Adjust stride and padding
        
    def forward(self, noise):
        noise = noise.view(-1, 1000)
        h = self.fc(noise)
        h = h.view(-1, 512, 4, 8, 8)  # Reshape to 512 channels of size 8x8x8
        h = F.relu(self.bn1(h))
