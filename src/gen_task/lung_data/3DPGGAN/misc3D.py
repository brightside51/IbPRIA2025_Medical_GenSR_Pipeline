...
def save_vol(images, vol_filename):
    temp = images.squeeze()
...
def create_image_grid(images, grid_size=None):
    ...
    grid = np.zeros(list(images.shape[2:-1]) + [grid_h * img_h, grid_w * img_w], dtype=images.dtype)
    print(grid.shape)
    for idx in range(num):
        #print(idx)
        x = (idx % grid_w) * img_w
        y = (idx // grid_w) * img_h
        grid[..., y : y + img_h, x : x + img_w] = images[idx, 0, :,:, np.round(img_d//2)]
    return grid
...
def save_numpy(images, filename, drange=[0,1], grid_size=None):
...
def save_image_grid(images, filename, drange=[0,1], grid_size=None):
    ...
    images = images.astype('float32') # imshow doesnt support lower
    ...
    for idx in range(images.shape[0]):
        # normalize, 0 .. 255
        #images[idx] = (images[idx] - np.min(images[idx]))/np.ptp(images[idx])
        #images[idx] = (images[idx] * 255).astype('uint16')

        plt.subplot(1, images.shape[0], idx+1)
        plt.axis('off')
        
        z_plot = int(images[idx].shape[3]/2)
        plt.imshow(images[idx, 0, :, :, z_plot], cmap="gray")

    plt.savefig(filename)
    plt.close()

    #save_vol(images, vol_filename)
    #convert_to_pil_image(create_image_grid(images, grid_size), drange).save(filename)

def save_image_grid_2d(images, filename, drange=[0,1], grid_size=None):
    #print(images.shape)
    #print(images.dtype)
    #print(filename)
    #print(type(filename))
...
def locate_result_subdir(run_id_or_result_subdir):
    ...
    searchdirs += ['networks']
    print(searchdirs)
    print('Searching ', run_id_or_result_subdir)
...
def locate_network_pkl(run_id_or_result_subdir_or_network_pkl, snapshot=None):
    print('locate_network_pkl 1', run_id_or_result_subdir_or_network_pkl, snapshot)
    ...
    print('locate_network_pkl 2', run_id_or_result_subdir_or_network_pkl, snapshot)
