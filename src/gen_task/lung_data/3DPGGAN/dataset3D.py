...
class Dataset_LIDC(Dataset):
    def __init__(self, root_path, image_size=128, num_frames=64) :
    ...
    def __getitem__(self, sampleIdx):
        ...
        # If less than 64 images, duplicate last until reaching max
        ...
        # If more than 64 images, get only middle section
        elif(num_mris > max_size):
            #start = (num_mris - max_size) // 2
            #end = start + max_size
            #tensors_filt = tensors_filt[start:end]
            tensors_filt = tensors_filt[0:max_size]
    ...
    def load_dataset(class_name='dataset_3D.TFRecordDataset', data_dir=None, verbose=False, **kwargs):
        ...
        dataset = tfutil_3D.import_obj(class_name)(**adjusted_kwargs)
        if verbose:
