...
def create_session(config_dict=dict(), force_as_default=False):
    ...
    config.gpu_options.allow_growth = True # not all full memory, on-demand allocation
    config.gpu_options.per_process_gpu_memory_fraction = 0.8 #limit GPU memory usage
...
class Optimizer:
                ...
                with tf.name_scope('ApplyGrads%d' % dev_idx), tf.device(dev):
                    ...
                    if self.use_loss_scaling or total_grads > 1:
                        with tf.name_scope('Scale'):
                            coef = tf.constant(np.float32(1.0 / total_grads), name='coef', dtype=tf.float32)
    ...
    def get_loss_scaling_var(self, device):
        ...
        print(self.name,'Loss Scaling:', np.float32(self.loss_scaling_init))
...
class Network:
    ...
    def __setstate__(self, state):
        ...
        module = importlib.new_module('_tfutil_network_import_module_%d' % len(_network_import_modules))
            ...
            with absolute_name_scope(self.scope + '/Run'), tf.control_dependencies(None):
                ...
                for gpu in range(num_gpus):
                    with tf.device('/gpu:%d' % gpu):
                        ...
                        if out_shrink > 1:
                            ksize = [1, 1, out_shrink, out_shrink, out_shrink]
                            out_expr = [tf.nn.avg_pool3D(x, ksize=ksize, strides=ksize, padding='VALID', data_format='NCDHW') for x in out_expr]
    ...
    """def print_layers(self, title=None, hide_layers_with_no_params=False):
        ...
        for variable in tf.trainable_variables():
            ...
            print(variable.name,  variable.shape, end='')
            #print(len(shape))
