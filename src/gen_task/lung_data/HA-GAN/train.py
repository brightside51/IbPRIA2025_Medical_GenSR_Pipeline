...
def main():
    ...
    if args.img_size == 256:
        from models.model_256 import Discriminator, Generator, Encoder, Sub_Encoder
    elif args.img_size == 128:
        from models.model_128_2 import Discriminator, Generator, Encoder, Sub_Encoder
    else:
    ...
    for iteration in range(args.continue_iter, args.num_iter):
       ...
       # randomly select a high-res sub-volume from real image
        ...
        start_idx = crop_idx
        end_idx = crop_idx + args.img_size // 8
        # Apply slicing operation
        real_images_crop = real_images[:, :, start_idx:end_idx, :, :]
        print("REAL_IMAGES_CROP")
        print(real_images_crop)
        ...
        if iteration%200 == 0:
            ...
            real_save_path = os.path.join(save_dir, "real_image" +str(iteration+1)+".png")
            plotting.plot_img(featmask, title="REAL", cut_coords=(args.img_size//2, args.img_size//2, args.img_size//16), draw_cross=False, cmap="gray")
            ...
            rec_save_path = os.path.join(save_dir, "reconstructed_image" +str(iteration+1)+".png")
            plotting.plot_img(featmask, title="REC", cut_coords=(args.img_size//2, args.img_size//2, args.img_size//16), draw_cross=False, cmap="gray")
            ...
            fake_save_path = os.path.join(save_dir, "fake_image" +str(iteration+1)+".png")
            plotting.plot_img(featmask, title="FAKE", cut_coords=(args.img_size//2, args.img_size//2, args.img_size//16), draw_cross=False, cmap="gray")
